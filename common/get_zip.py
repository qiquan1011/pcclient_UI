...
def writeAllFileToZip(absDir, zipFile):
    for f in os.listdir(absDir):
        ...
        #abs_path=absFile.replace(zipFilePath,"")
        #print(absFile)
        if os.path.isdir(absFile):  # 判断是文件夹，继续深度读取。
            #relFile = absFile[len(os.getcwd()) + 2:]  # 改成相对路径，否则解压zip是/User/xxx开头的文件。
            #print(relFile)
            zipFile.write(absFile)  # 在zip文件中创建文件夹
            writeAllFileToZip(absFile, zipFile)  # 递归操作
        else:  # 判断是普通文件，直接写到zip文件中。
            #relFile = absFile[len(os.getcwd()) + -7:]  # 改成相对路径
            #print(relFile)
            zipFile.write(absFile)
    return


zipFilePath = os.path.join(sys.path[0], "allure-report-html.zip")
...
absDir = "D:/pythonProject/pcclient_UI/result/allure-report-html"
# 要压缩的文件夹绝对路径。
